...
As of the npm `@flue/runtime@0.9.1` / `@flue/cli@0.9.1` release, Hatchery still needs its own
...
- Move the external coding runner shape to a separate Flue workflow when we want workflow run IDs,
  event streams, logs, typed results, and sandbox staging owned outside the Hatchery Worker.
- Use Flue subagents inside the runner/controller session for mini-swarm behavior. Do not persist
  controller topology in Hatchery; keep Hatchery to receipts, routes, dedupe, wake policy, and
  notifications.
- Revisit Cloudflare `cloudflare.ts` extension hooks only when Hatchery needs native lifecycle
