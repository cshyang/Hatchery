...
In `.dev.vars`, remove the line `NONGO_AAPI_KEY="<redacted>"` and add (use the operator's real Nango DEV API key for `NANGO_SECRET_KEY`, and the Nango webhook signing key once registered — placeholder until then):
...
> The value `<redacted>` was the half-set Nango key under a typo'd name; carry it over to `NANGO_SECRET_KEY` only if it is in fact the DEV secret key (confirm in the Nango UI — Nango distinguishes a public key from a secret key).
