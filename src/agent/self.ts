...
export function buildSelfStatus(input: SelfStatusInput) {
  ...
  return {
    ...
    capabilities: {
      ...
      agentRuns: capability(
        ...
        input.hasAgentRunner
          ? 'Admin-approved routes can turn Linear state transitions into Hatchery agent-run receipts and dispatch an external E2B/Pi runner with Agent Kits. Boundary events are stored in the event ledger; route activation is admin-only.'
    ...
    limits: [
      ...
      'Linear-driven coding work is control-plane only: Hatchery records leases and callbacks; the external E2B runner owns Pi execution, Agent Kit loading, clone/edit/test/commit/PR, and never auto-merges from this runtime.',
